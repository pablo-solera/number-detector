...
lower_blue = np.array([112, 167, 48], dtype=np.uint8)
...
def preparar_roi_para_ocr(roi_bgr):
    ...
    return thr
def parsear_motor(texto):
    """
    Convierte el bloque OCR en un diccionario de motor
    """
    ...
    # motor tipo 1.4/DGEA
    m = re.search(r"\d\.\d/[A-Z0-9]+", texto)
    if m:
        motor = m.group()

    # potencia
    m = re.search(r"kW[: ]*(\d+)", texto, re.IGNORECASE)
    if m:
        kw = int(m.group(1))

    m = re.search(r"Cv[: ]*(\d+)", texto, re.IGNORECASE)
    if m:
        cv = int(m.group(1))

    # id vehículo
    m = re.search(r"idVeic[: ](\d+)", texto, re.IGNORECASE)
    if m:
        idVehiculo = m.group()
    print(m)
    return {
...
for cnt in contours:
    ...
    if texto:
        ...
        motor_data = parsear_motor(texto)
        print(texto)

        # solo guardar si detectamos motor
        if motor_data["motor"]:
            resultados.append(motor_data)
